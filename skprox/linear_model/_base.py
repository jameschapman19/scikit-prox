...
class RegularisedRegression(LinearRegression):
    def __init__(self, fit_intercept=True, copy_X=True, proximal='Dummy', proximal_params=None, sigma=1.0, rho=0.8,
                 radius=1.0, gamma=1.3, delta=1e-10,
                 positive=False, max_iter=1000, tol=1e-4, learning_rate=0.01):
    ...
    def _proximal_gradient_descent(self, X, y):
        ...
        for i in range(self.max_iter):
            ...
            coef_ = self.proximal.prox((coef_.flatten() - self.learning_rate * grad.flatten()), self.sigma).reshape(
                coef_.shape)
        return coef_
    ...
    def _get_proximal(self):
        if callable(self.proximal):
            ...
            params = {"ndim": self.ndim, "sigma": self.sigma, "isotropic": True, "positive": self.positive,
                      "dims": self.dims, "rho": self.rho, "dim": self.dims, "radius": self.radius, "gamma": self.gamma,
                      "delta": self.delta}
...
if __name__ == '__main__':
    ...
    X, y, t = make_regression(n_samples=100, n_features=200, n_informative=10, n_targets=5, random_state=1, coef=True)
